...
\begin{frame}
	...
	\begin{tikzpicture}
		...
		\structcell[padding]{} ;
		\structcell[padding]{} ;
		\structcell[occupiedcell]{\texttt{not\_my\_key}} ;
		\structcell[occupiedcell]{\texttt{not\_my\_key}} ; \coordinate(fourthCellOut) at (currentcell.east);
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item Requires: \texttt{offset} and \texttt{capacity} coprime ($gcd = 1$).
	\end{itemize}
\end{frame}

\begin{frame}
	\frametitle{Goal}
	Insert \texttt{key4}: $h_1(\mathtt{key4}) = 5$, $h_2(\mathtt{key4}) = 2$.
	\begin{tikzpicture}
		\draw (0, -5) node[anchor=east] (key4) {$h(\mathtt{key4})$};
		...
		\structcell[occupiedcell]{\keyvalue{key2}{value2}} \coordinate(fifthCell) at (currentcell.west); \coordinate(fourthCellOut) at (currentcell.east);
		\structcell[freecell]{\keyvalue{}{}} \coordinate(sixthCell) at (currentcell.west); \coordinate (sixthCellOut) at (currentcell.east);
