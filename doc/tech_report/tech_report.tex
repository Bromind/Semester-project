...
Second, formally proving that the map is correct. The semantic of \emph{correctness} is the same as the previous one. The only difference in the specification is the requirement of coprime capacity and offset, which is easily achieved, for instance taking a capacity $2^n$ and an odd offset.

The formal proof has been completed, except for one lemma: 
